...
import com.mb.scrapbook.app.module.example.lib.recyclerview.view.JDAccessibilityFragment
...
class ExampleRepository(private val state: MutableLiveData<State>): BaseRepository() {
    ...
    companion object {
        ...
        const val TYPE_SYSTEM: Int = 0x0001
    ...
    init {
         ...
         * 系统设置
         */
        val listSystem: MutableList<ExampleItemData> = mutableListOf(
            ExampleItemData((TYPE_SYSTEM + genKey.incrementAndGet()),
                             name = "JD日常巡检",
                             groupType = TYPE_SYSTEM,
                             hasChild = false,
                             fragment = JDAccessibilityFragment())
        )

        /**
         * 指示器项
         */
        val listIndicator: MutableList<ExampleItemData> = mutableListOf(
         ...
         * RecyclerView项
         */
        val listRecyclerView: MutableList<ExampleItemData> = mutableListOf(
        ...
        mapData = mutableMapOf(
            Pair(ItemKey(TYPE_SYSTEM, "系统服务"), listSystem),
    ...
    private fun findKey(type: Int): ItemKey? {
        return mapData.keys.filter { it.id == type }[0]
    }
}